export const calcBookingPrice = (catalog, resortId, weekId, duration, adults, children) => {
  try {
    let price = 0
...
const pricingForResortAndWeek = (catalog, resortId, weekId) => {
  return catalog.pricing.find(p => p.resorts.includes(resortId) && p.grids.weeks.includes(weekId))
}
